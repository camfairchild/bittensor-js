...
export class DelegatesRegistry {
  ...
  public async getDelegatesInfoWithExtras<ApiType extends ApiPromise>(
    api: BittensorApiPromise<ApiType>
  ): Promise<Array<DelegateExtra & DelegateInfo>> {
    const delegatesInfo = await api.getDelegateInfo()
    const delegatesJson = await this.getDelegatesJson()

    const delegatesInfoWithExtras = delegatesInfo.map(
      (delegateInfo: DelegateInfo) => {
        const delegateJson: DelegateExtra | undefined =
          delegatesJson[delegateInfo.delegate_ss58]
        return {
          ...delegateInfo,
          ...delegateJson,
        }
